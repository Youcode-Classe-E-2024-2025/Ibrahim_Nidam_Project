...
<body class="bg-gradient-to-tr from-[#2a464e] via-[#243b42] to-[#1d2b31] text-gray-200 w-screen h-screen overflow-auto">
    ...
    <!-- PAGE TITLE & ADD PROJECT BUTTON -->
    <div class="px-10 mt-6">
        <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold">Projects</h1>
        <!-- Add Project Button -->
        <button
            class="flex items-center gap-2 py-2 px-4 bg-indigo-600 rounded hover:bg-indigo-700 text-white font-semibold transition-colors"
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 px-10 mt-8">
        ...
        <div class="bg-white bg-opacity-90 rounded-lg p-5 text-gray-800 shadow-md hover:bg-opacity-100 transition-colors relative group">
        <!-- Lock icon (toggling) -->
        <button
        ...
        <h2 class="text-lg font-bold text-gray-800 mb-2">Example Project #1</h2>
        <!-- Completion Bar -->
        <div class="text-sm text-gray-600 mb-3">Completion: 45%</div>
        <div class="w-full bg-gray-200 rounded-full h-2 mb-3">
            <div class="bg-indigo-500 h-2 rounded-full" style="width: 45%;"></div>
        </div>
        <p class="text-sm text-gray-600">Project description goes here. It's a wonderful project with lots of potential.</p>
        </div>
        ...
        <h2 class="text-lg font-bold mb-2">Example Project #2</h2>
        <!-- Completion Bar -->
        <div class="text-sm text-gray-600 mb-3">Completion: 80%</div>
        <div class="w-full bg-gray-200 rounded-full h-2 mb-3">
            <div class="bg-green-500 h-2 rounded-full" style="width: 80%;"></div>
        </div>
        <p class="text-sm text-gray-600">Another awesome project with a higher completion percentage!</p>
        </div>
        
        <!-- Feel free to duplicate or loop through your real projects here -->
        <!-- ... -->

    </div>

    <!-- SIMPLE SCRIPT TO TOGGLE LOCK ICON -->
    <script>
        function toggleLock(btn) {
        const isLocked = btn.getAttribute('data-locked') === 'true';
        const svg = btn.querySelector('.lock-icon');
        
        // If locked, switch to unlocked icon
        if (isLocked) {
            svg.innerHTML = `
            <path d="M17 8V7c0-2.757-2.243-5-5-5S7 4.243 
                7 7v1H6c-1.103 0-2 .897-2 
        ...
        // If unlocked, switch to locked icon
        else {
            svg.innerHTML = `
